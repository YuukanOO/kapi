...
/**
 * This is the context of the kapi environment.
 */
const kapi = (function() {
    function Kapi() {
        this._onStart = {};
    ...
    Kapi.prototype.onStart = function(callback) {
...
fs.readdirSync(pluginsPath).map((ppath) => {
    const pluginName = path.basename(ppath, path.extname(ppath));
    plugins[pluginName] = require(path.join(pluginsPath, ppath));
});

/**
 * Informs all plugins that kapi is ready to start!
 */
for(let pname in plugins) {
    const plugin = plugins[pname];

    plugin.call(plugin, kapi);
}
